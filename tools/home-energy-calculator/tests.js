...
await test("Zero quantity is rejected", () => { try { calculateHomeEnergy({ ...base, appliances: [{ ...base.appliances[0], quantity: "0" }] }); } catch (error) { assert(error instanceof ValidationError); return; } throw new Error("Expected validation error"); });
...
await test("Zero days produces zero usage", () => closeTo(calculateHomeEnergy({ ...base, appliances: [{ ...base.appliances[0], daysPerMonth: "0" }] }).monthlyKwh, 0));
